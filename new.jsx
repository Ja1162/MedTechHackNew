...
const C = {
  ...
  patientColor: "#FF4757",
  familyColor: "#4361EE",
  caregiverColor: "#00C2A8",
};

const G = {
  coral: "linear-gradient(135deg, #FF4757 0%, #FF6B35 100%)",
  blue: "linear-gradient(135deg, #4361EE 0%, #7B2FBE 100%)",
  teal: "linear-gradient(135deg, #00C2A8 0%, #00B4D8 100%)",
  yellow: "linear-gradient(135deg, #FFD700 0%, #FF6B35 100%)",
  pink: "linear-gradient(135deg, #F72585 0%, #7B2FBE 100%)",
  green: "linear-gradient(135deg, #2DC653 0%, #00C2A8 100%)",
  hero: "linear-gradient(160deg, #FAEBD7 0%, #FAEBD7 40%, #FAEBD7 100%)",
};
...
const globalStyles = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  ::-webkit-scrollbar-track { background: #FAEBD7; }
  ::-webkit-scrollbar-thumb { background: #FAEBD7; border-radius: 4px; }
...
const S = {
  ...
  card: (accent) => ({
    ...
    borderRadius: 24,
    padding: "32px 36px",
    boxShadow: accent ? `0 4px 0 ${accent}22, 0 8px 30px ${accent}08` : `0 2px 0 #FAEBD7, 0 8px 24px rgba(0,0,0,0.04)`,
  }),
...
const BASELINE = 72;
const MEMORY_IMAGES = ["🍎","🎯", "🚀", "🌟"];
const WORDS = [{ scrambled: "YOMEMR", answer: "MEMORY" }, { scrambled: "NRIAB", answer: "BRAIN" }, { scrambled: "GCEOIVNIT", answer: "COGNITIVE" }];
const BRAIN_FOODS = [
  ...
  return (
    <div style={{ marginBottom: 28 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <span style={{ fontSize: "1rem", fontWeight: 600, color: C.muted }}>{icon} {label}</span>
...
function HomeScreen({ user, checkIn, setCheckIn, onStart }) {
  ...
  return (
    <div style={S.page}>
      <div style={{ background: `linear-gradient(135deg, #FFF8F0 0%, #F0F4FF 50%, #F5F0FF 100%)`, borderBottom: `2px solid ${C.border}`, padding: "56px 48px 48px" }}>
            ...
            {moodEmojis.map(([em, lbl], i) => {
              ...
              return (
                <button key={i} onClick={() => setCheckIn(c => ({ ...c, mood: (i + 1) * 2 }))} style={{
                  ...
                  boxShadow: isActive ? "0 6px 20px rgba(0,0,0,0.13)" : "none",
        ...
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 24 }}>
          <div className="ai2" style={S.card()}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 40, marginTop: 12 }}>
              <EmojiSlider label="Energy" value={checkIn.energy} onChange={v => setCheckIn(c => ({ ...c, energy: v }))} icon="⚡" color={C.orange} gradient={G.yellow} />
        ...
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 28 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            <div className="ai2" style={S.card(C.sky)}>
              <label style={{ ...S.label, color: C.sky }}>How Did Today Feel?</label>
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ ...S.container, padding: "40px 48px 80px" }}>
        ...
        {phase === "pattern" && <PatternTask onDone={s => finish("pattern", s)} />}
        {phase === "alternating" && <AlternatingTask onDone={s => finish("alternating", s)} />}
        {phase === "scramble" && <ScrambleTask onDone={s => finish("scramble", s)} />}
        {phase === "results" && (
        ...
        {stage === "show" ? (
          <>
            ...
            <div style={{ display: "flex", gap: 24, justifyContent: "center" }}>
              {shown.map((em, i) => (
                <div key={i} style={{ width: 100, height: 100, background: "rgba(255,255,255,0.25)", borderRadius: 20, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "3rem", opacity: revealed.includes(em) ? 1 : 0, transition: "opacity 0.4s" }}>{em}</div>
              ))}
            </div>
          </>
        ) : (
          <>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 14, justifyContent: "center", marginBottom: 24 }}>
              {all.map(em => (
                <button key={em} onClick={() => toggle(em)} style={{ width: 80, height: 80, fontSize: "2.5rem", background: selected.includes(em) ? "rgba(255,255,255,0.4)" : "rgba(255,255,255,0.15)", border: `3px solid ${selected.includes(em) ? "rgba(255,255,255,0.9)" : "rgba(255,255,255,0.3)"}`, borderRadius: 18, cursor: "pointer", transform: selected.includes(em) ? "scale(1.1)" : "scale(1)", transition: "all 0.15s" }}>{em}</button>
              ))}
            </div>
            <button style={{ ...S.btn("rgba(255,255,255,0.9)", C.coral), padding: "16px 40px", fontSize: "1.1rem", margin: "0 auto" }} onClick={submit}>Submit Answers ✓</button>
          </>
        )}
      </div>
    </div>
...
function PatternTask({ onDone }) {
  const correct = [1, 4, 7, 8, 9, 6, 3, 2, 5];
...
function AlternatingTask({ onDone }) {
  const correct = [1,"B","A",4,2,3,"D","C",5];
  const [path, setPath] = useState([]);
  const [done, setDone] = useState(false);
  const tap = n => {
    if (done || path.includes(n)) return;
      ...
      <div style={{ ...S.container, padding: "40px 48px 100px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 28 }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div className="ai1" style={S.card(C.coral)}>
              <label style={{ ...S.label, color: C.coral }}>Language</label>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {languages.map(l => <Chip key={l} label={l} active={language === l} onClick={() => setLanguage(l)} color={C.coral} gradient={G.coral} />)}
              </div>
            </div>
